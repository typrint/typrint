...
class Init extends AbstractMigration
{
    public function change(): void
    {
        $this->table(DB_TABLE_PREFIX.'users', ['id' => false, 'primary_key' => ['id']])
            ...
            ->addTimestamps()
            ->addIndex('name', ['unique' => true, 'name' => 'idx_'.DB_TABLE_PREFIX.'users_name'])
            ->addIndex('email', ['unique' => true, 'name' => 'idx_'.DB_TABLE_PREFIX.'users_email'])
        ...
        $this->table(DB_TABLE_PREFIX.'contents', ['id' => false, 'primary_key' => ['id']])
            ...
            ->addColumn('password', 'string', ['limit' => 255])
            ->addTimestamps()
            ->addIndex(['name', 'type'], ['unique' => true, 'name' => 'idx_'.DB_TABLE_PREFIX.'contents_name_type'])
            ->addIndex(['parent', 'type', 'status'], ['name' => 'idx_'.DB_TABLE_PREFIX.'contents_parent_type_status'])
            ->addIndex(['type', 'status', 'created_at', 'user_id'], ['name' => 'idx_'.DB_TABLE_PREFIX.'contents_type_status_created_at'])
            ->addIndex(['user_id', 'type', 'status', 'created_at'], ['name' => 'idx_'.DB_TABLE_PREFIX.'contents_user_id_type_status'])
            ->create();
        ...
        $this->table(DB_TABLE_PREFIX.'term_taxonomy', ['id' => false, 'primary_key' => ['id']])
            ...
            ->addIndex(['term_id', 'taxonomy'], ['unique' => true, 'name' => 'idx_'.DB_TABLE_PREFIX.'term_taxonomy_term_id_taxonomy'])
            ->create();
